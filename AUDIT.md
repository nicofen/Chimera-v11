...
from chimera.agents.risk_agent import RiskAgent, MAX_KELLY_FRACTION, MIN_CONFIDENCE
from chimera.utils.state import SharedState, TechnicalSignals, Sentiment
from chimera.utils.state import NewsState
...
class TestPositionSizing:
    ...
    def test_position_size_formula(self, agent_and_state, config):
        ...
        asyncio.get_event_loop().run_until_complete(agent._process(sig))
    ...
    def test_veto_blocks_order(self, agent_and_state, signal):
        ...
        asyncio.get_event_loop().run_until_complete(agent._process(signal))
        assert len(orders) == 0
